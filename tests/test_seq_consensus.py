...
class TestAlignmentFuncs(unittest.TestCase):
    ...
    def test_consensus(self):
        seqs = ["GTN-CTA--", "GC--TCAA-", "-CT-CTAA-", "ACCACYAA-"]
        exp_cons = [
        ...
        for threshold, free_cons, cons, strip_cons in exp_cons:
            calc_cons = consensus(
                seqs, threshold, free_endgaps=True, end_gap_char_out="."
            )
            self.assertEqual(calc_cons, free_cons)
            calc_cons2 = consensus(
                seqs, threshold, free_endgaps=False, end_gap_char_out="."
            )
            self.assertEqual(calc_cons2, cons)
            calc_cons3 = consensus(seqs, threshold, free_endgaps=True, strip_gaps=True)

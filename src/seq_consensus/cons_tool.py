...
from seq_consensus import AlignmentFrequencies, IUPAC_map, AlphabetLookupError
...
try:
    ...
    print("Biopython should be installed in order to use 'cons_tool.py'", file=stderr)
    exit(1)
...
def calc_consensus(
    ...
    if out_format == "fasta":
        ...
        def writer(head, seq):
            out.write_record(SeqRecord(Seq(seq), id=head, description=""))

    elif out_format == "line":

        def writer(_, seq):
            output.write(seq + linesep)
    ...
    for aln_file in aln_files:
        ...
        if key is None:
            ...
            if min_coverage is not None:
                ...
                cons = "".join(lt for lt, include in zip(cons, keep) if include)
            writer(_header, cons)
        else:
            ...
            if min_coverage is not None:
                cov = np.stack([freqs.coverage() for freqs in group_freqs.values()])
...
parser = argparse.ArgumentParser(
    ...
    """.format(
        pkg_resources.get_distribution("seq_consensus").version
    )
)
...
parser.add_argument(
    "-f",
    "--out-format",
    choices={"line", "raw", "line/fasta", "raw/fasta", "fasta"},
    default="line/fasta",
    help="""
...
parser.add_argument(
    ...
    "--no-free-endgaps",
    action="store_true",
    help="Do not ignore end gaps when calculating the" "consensus.",
)
parser.add_argument(
    "-g", "--gap-char", default="-", help='Gap character in alignment (default: "-").'
)
parser.add_argument(
    "-e",
    "--end-gap-char",
...
parser.add_argument(
    ...
    "--wrap",
    type=int,
    help="Wrap FASTA lines in the output" "(default: no wrapping)",
)
...
if args.key is not None or len(args.alignment) > 1:
    if "fasta" not in outfmt:
        print(
            "Warning: -f line or -f raw are incompatible with --key "
...
try:
    ...
    if args.gap_char == "-" and e.letters == ["."]:
        print(
            'cons_tool.py: "." encountered in the input. If end gaps are'
            "different, specify with -e/--end-gap-char"
        )
    else:
        print(
            """cons_tool.py: Invalid character(s) in the input: "{}".
